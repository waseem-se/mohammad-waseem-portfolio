        ...
        highlights: [
          {
            ...
            impact: {
              value: "20%",
              label: "Shorter development cycle",
              kind: "reduction",
            },
        ...
        highlights: [
          {
            ...
            impact: {
              value: "18%",
              label: "better predictive accuracy",
              kind: "accuracy",
            },
            tech: ["C#", "ASP.NET"],
          },
...
const roleSeriesById: ReadonlyMap<string, RoleSeries> = (() => {
  const map = new Map<string, RoleSeries>(
    roleSeries.map((series) => [series.id, series]),
  );
  const roleIds = companies.flatMap((company) =>
  ...
  for (const id of roleIds) {
    if (!map.has(id)) {
      throw new Error(
        `roleSeries has no entry for role "${id}" — charts would mis-colour it.`,
      );
    }
  }
  for (const series of roleSeries) {
    if (!roleIds.includes(series.id)) {
      throw new Error(
        `roleSeries has an entry for "${series.id}", which is not a role.`,
      );
    }
  }
